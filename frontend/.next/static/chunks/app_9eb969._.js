(globalThis.TURBOPACK = globalThis.TURBOPACK || []).push(["static/chunks/app_9eb969._.js", {
...
function Depth({ market }) {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "Depth.useEffect": ()=>{
            __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$utils$2f$SignalingManager$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["SignalingManager"].getInstance().registerCallback("depth", {
                "Depth.useEffect": (data)=>{
                    console.log("depth has been updated");
                    console.log(data);
                    setBids({
                        "Depth.useEffect": (originalBids)=>{
                            ...
                            for(let j = 0; j < data.bids.length; j++){
                                if (Number(data.bids[j][1]) != 0 && !bidsAfterUpdate.map({
                                    "Depth.useEffect": (x)=>x[0]
                                }["Depth.useEffect"]).includes(data.bids[j][0])) {
                                    bidsAfterUpdate.push(data.bids[j]);
                                    break;
                                }
                            }
                            bidsAfterUpdate.sort({
                                "Depth.useEffect": (x, y)=>Number(y[0]) > Number(x[0]) ? -1 : 1
                            }["Depth.useEffect"]);
                            return bidsAfterUpdate;
                        }
                    }["Depth.useEffect"]);
                    setAsks({
                        "Depth.useEffect": (originalAsks)=>{
                            ...
                            for(let j = 0; j < data.asks.length; j++){
                                if (Number(data.asks[j][1]) !== 0 && !asksAfterUpdate.map({
                                    "Depth.useEffect": (x)=>x[0]
                                }["Depth.useEffect"]).includes(data.asks[j][0])) {
...
_s(Depth, "YlQSi6ITQ0idEraDsI4BxAb5IFA=");
_c = Depth;
...
//# sourceMappingURL=app_9eb969._.js.map